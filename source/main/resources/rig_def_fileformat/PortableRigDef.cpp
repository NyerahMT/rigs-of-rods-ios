/*
    ...
    Portable structural subset of the native RigDef parser for iOS.
*/
...
enum class Section { None,Globals,Nodes,Beams,Hydros,Shocks,Shocks2,Wheels,Wheels2,Engine,Brakes,Contacters,Unknown };
...
Section S(const std::string&k){if(k=="globals")return Section::Globals;if(k=="nodes"||k=="nodes2")return Section::Nodes;if(k=="beams")return Section::Beams;if(k=="hydros")return Section::Hydros;if(k=="shocks")return Section::Shocks;if(k=="shocks2")return Section::Shocks2;if(k=="wheels")return Section::Wheels;if(k=="wheels2")return Section::Wheels2;if(k=="engine")return Section::Engine;if(k=="brakes")return Section::Brakes;if(k=="contacters")return Section::Contacters;return Section::Unknown;}
...
Document Parse(const std::string&text){Document d;BeamDefaults bd;Section sec=Section::None;std::istringstream in(text);std::string raw;int ln=0;while(std::getline(in,raw)){++ln;auto line=StripComments(raw);if(line.empty())continue;if(d.name.empty()){d.name=line;continue;}auto t=Tokens(line);if(t.empty())continue;auto k=Lower(t[0]);if(k=="end")break;
if(k=="set_beam_defaults"){if(t.size()>=5){bd.spring=F(t[1],9000000);bd.damping=F(t[2],12000);bd.deform=F(t[3],400000);bd.strength=F(t[4],100000);}else W(d,ln,"set_beam_defaults requires four values");continue;}
if(k=="set_node_defaults"){if(t.size()>=2){d.node_defaults.load_weight=F(t[1],0);d.node_defaults.load_weight_is_set=d.node_defaults.load_weight>0;if(t.size()>=3)d.node_defaults.minimass=F(t[2],50);}else W(d,ln,"set_node_defaults requires load weight");continue;}
...
case Section::Nodes:if(t.size()>=4){Node n;n.id=t[0];n.x=F(t[1]);n.y=F(t[2]);n.z=F(t[3]);if(t.size()>4)n.options=t[4];n.load_weight=d.node_defaults.load_weight;n.load_weight_is_set=d.node_defaults.load_weight_is_set;n.minimass=d.node_defaults.minimass;d.nodes.push_back(n);}else W(d,ln,"node requires id,x,y,z");break;
...
case Section::Engine:if(t.size()>=4){d.engine.present=true;d.engine.shift_down_rpm=F(t[0]);d.engine.shift_up_rpm=F(t[1]);d.engine.torque=F(t[2]);d.engine.differential_ratio=F(t[3],1);for(size_t i=4;i<t.size();++i)d.engine.gear_ratios.push_back(F(t[i]));}break;
