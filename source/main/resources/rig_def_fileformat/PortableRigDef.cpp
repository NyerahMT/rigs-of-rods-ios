...
case Section::Engine:if(t.size()>=6){d.engine.present=true;d.engine.shift_down_rpm=F(t[0]);d.engine.shift_up_rpm=F(t[1]);d.engine.torque=F(t[2]);d.engine.differential_ratio=F(t[3],1);d.engine.reverse_gear_ratio=F(t[4]);d.engine.neutral_gear_ratio=F(t[5],1);d.engine.gear_ratios.clear();bool terminated=false;for(size_t i=6;i<t.size();++i){float ratio=F(t[i]);if(ratio<0){terminated=true;break;}d.engine.gear_ratios.push_back(ratio);}if(!terminated)W(d,ln,"engine forward gears should end with -1 terminator");if(d.engine.gear_ratios.empty())W(d,ln,"engine requires at least one forward gear");}else W(d,ln,"engine requires shift RPMs, torque, differential, reverse and neutral ratios");break;
