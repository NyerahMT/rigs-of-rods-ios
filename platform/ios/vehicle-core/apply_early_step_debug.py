...
if "AuthoredVehicleRuntime::EarlyDiagnostics() const" in text:
    print(f"Early physics instrumentation already present in {p}"); raise SystemExit(0)
def r(old,new,label):
    global text
    n=text.count(old)
...
r('        for (BeamLink& link : beams)\n        {\n            ApplyBeamForce(nodes[link.a], nodes[link.b], link.beam);\n            if (!std::isfinite(link.beam.stress))\n','        std::size_t debug_worst_beam = kInvalidIndex;\n        float debug_worst_stress = 0.0f;\n        for (std::size_t beam_index = 0; beam_index < beams.size(); ++beam_index)\n        {\n            BeamLink& link = beams[beam_index];\n            ApplyBeamForce(nodes[link.a], nodes[link.b], link.beam);\n            if (std::isfinite(link.beam.stress) && std::fabs(link.beam.stress) > std::fabs(debug_worst_stress)) { debug_worst_stress=link.beam.stress; debug_worst_beam=beam_index; }\n            if (!std::isfinite(link.beam.stress))\n','beam scan')
r('        ++physics_steps;\n    }\n};\n','        ++physics_steps;\n        if (physics_steps <= 2)\n        {\n            EarlyStepDiagnostics d; d.step=physics_steps; float worst=-1.0f;\n            for (std::size_t i=0;i<nodes.size() && i<debug_start_positions.size();++i)\n            {\n                const float disp=Distance(nodes[i].position,debug_start_positions[i]);\n                if(disp>worst){worst=disp;d.worst_node=i;d.start_position=debug_start_positions[i];d.position=nodes[i].position;d.velocity=nodes[i].velocity;d.force=nodes[i].force;d.mass=nodes[i].mass;d.displacement=disp;}\n            }\n            d.worst_beam=debug_worst_beam;\n            if(debug_worst_beam!=kInvalidIndex && debug_worst_beam<beams.size()){const BeamLink& b=beams[debug_worst_beam];d.beam_a=b.a;d.beam_b=b.b;d.beam_stress=b.beam.stress;d.beam_length=Distance(nodes[b.a].position,nodes[b.b].position);d.beam_rest_length=b.beam.rest_length;d.beam_spring=b.beam.spring;d.beam_damping=b.beam.damping;}\n            early_diagnostics.push_back(d);\n        }\n    }\n};\n','capture')
...
p.write_text(text); print(f"Instrumented early physics steps in {p}")
